...
import { CheckObjOrArrForNull, capitalize, isDateValid, isNegativeNumber, isNullOrUndefined, isStrEmpty  } from '@utils/helpers';
...
/// types
import { MtrlTableAllAdminUsers } from '@app/api/Types/queryReturnTypes/Admin';
import { FieldType, Localization } from '@app/redux/types/ReportTypes';
import { AdminFilterOptions } from '@app/Types/utils';
// svgs
import EditSvg from '@app/assets/customIcons/edit.svg'
import trashSvg from '@app/assets/customIcons/trash.svg'
import addUsrSvg from '@app/assets/customIcons/addUser.svg'
...
type TableProps = {
    ...
    /** @defaultValue 250px */
    heightToExtract?: '600' | '550' | '500' | '475' | '400' | '370' | '350'| '270' | '250' | '232'| '225' | '200' | '150' | '100' 
    /** @defaultValue 300 */
    columnSize?: 25 | 50 | 80 | 100 | 180 | 200 | 225 | 275 | 300
    /** for report page */
    ...
    onRowActionBalance?: (data : MtrlTableAllAdminUsers[]) => void
    onRowActionsSubsc?: (data : MtrlTableAllAdminUsers[]) => void
    /** @defaultValue false */
...
const MaterialTable = (props: TableProps) => {
  ...
  const tableCols = useMemo((): TableColType[] => {
    return isNullOrUndefined(field) ? tableLocalization[paperData.paperName][field?.value] 
    ...
    if(CheckObjOrArrForNull(finding)){
      ...
      return tableCols?.map(item => {
        return {
          ...
          // size: columnSize,   needa work on it later
          Cell: ({ cell }: {cell:any}) => {
            ...
            const connectedSt = id==='is_connected'

            // console.log('id', cell)

            if(activeSt||cancelledSt||connectedSt||activeUsrSt||usrConfirmed){
                ...
                return (
                  <div className={styles.statuses}>
                    <span
                      className={cx({
                        status: true,
                        [`state${capitalize(value)}`]: cancelledSt,
                        [`active${capitalize(value)}`]:  (connectedSt||activeSt||activeUsrSt||usrConfirmed)
                      })}
                    ></span>
                    <div>
                      {
                        realVal&&activeSt ? translation('active'):
                        realVal&&connectedSt ? translation('active') : 
                        realVal&&cancelledSt ? translatio('cancelled') : 
                        realVal&&activeUsrSt ? translation('active') : 
                        realVal&&usrConfirmed ? translation('confirmed') :
                        !realVal&&activeSt ? translation('inactive') :
                        !realVal&&connectedSt ? translation('inactive') : 
                        !realVal&&cancelledSt ? translation('notCancelled') : 
                        !realVal&&activeUsrSt ? translation('inactive') :
                        !realVal&&usrConfirmed ? translation('notConfirmed') : ""
                      }
                    </div>
                  </div>
                )
              ...
              id==='last_conn_dt')
                ...
                return (
                  <div className={cx({
                    redify: isNegativeNumber(cell.getValue())
                  })}>
                    {cell.getValue()}
                  </div>
                )
            ...
            <ThemeProvider theme={createTheme(tableTheme, )}>
              <MaterialReactTable
                ...
                renderRowActions={({row, table}) => {
                  return (
                    <div className={styles.rowActionEdit}
                      onClick={() => {
                      ...
                      <div className={cx({
                      ...
                      })}>
                        {
                          ...
                          <div className={styles.rowActionsBtn}>
                            <Button 
                              ...
                              color='theme' 
                              rounded
                              onClick={() => onRowActionBalance(table.getSelectedRowModel()?.flatRows?.map(row => {
                                ...
                                return {
                                  ...
                                  balance_sum: isNullOrUndefined(item.user_balance) && isNegativeNumber(item.user_balance) ? 0 : item.user_balance, 
                                  red_target: false, 
                                }
                              }))}
                            >
                            ...
                            <Button 
                              ...
                              color='theme' 
                              rounded
                              onClick={() => onRowActionsSubsc(table.getSelectedRowModel()?.flatRows?.map(row => {
                                const item = row.original
                                return {
                                  user_guid: item.user_guid, 
                                  user_name:item.user_name, 
                        ...
                        }
                        {
                          ...
                          <div className={styles.adminActionsWrapper}>
                            <div style={{marginLeft: '6px'}} onClick={() => onAdd(userGuids)}>
                              <img src={addUsrSvg}/>
                            </div>
                            {
                            ignoreAdminRemoveBtn ? 
                            "" : 
                            <div onClick={() => onRemove(userGuids)}>
                              <img src={trashSvg}/>
                            </div>
                            }
                          </div>
                  ...
                  sx: { 
                    ...
                    "&::-webkit-scrollbar": {
                      ...
                      "&::-webkit-scrollbar-thumb": {
                        backgroundColor: theme.mode === 'theme-mode-light' ? 'rgba(0, 0, 0, 0.54)' : '#edf2f7',
                        borderRadius: 10
                      }
                  } 
                }}
