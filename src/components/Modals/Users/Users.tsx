...
import { AdminSubscribedUsers, AdminUsers } from '@app/api/Types/queryReturnTypes/Admin';
                ...
                onAdd={async (userGuids) => {
                    if(CheckObjOrArrForNull(userGuids)){
                        try {
                            ...
                            const response = await assignUsersToFirm(dataToPost)
                            onSuccess()
                            setShow(false)
                    ...
                    }else toast.error('Select at least a user.')
                }}
            />
        </div>
    </Modal> 
