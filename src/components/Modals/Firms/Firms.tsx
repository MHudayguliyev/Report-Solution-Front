...
// utils 
import { checkIfArray,  CheckObjOrArrForNull, arraysChecker, delay, getDate, isDateValid, isNullOrUndefined, isStrEmpty  } from '@utils/helpers'
                                ...
                                <div className={cx({
                                    ...
                                    <div className={styles.radio}>
                                        <input 
                                            ...
                                            onChange={() => setOptionFN({connected: item.connected, value: item.value, label: item.label})}
                                        />
                                        <div
                                        onClick={() => {
                                            if(item.connected)
                                    ...
                                    <div className={styles.contents}>
                                        <div className={styles.upper}>
                                            ...
                                            <div className={styles.firm_tels}>
                                                <div>
                                                    {item.firm_tel_num1} 
                                                    {isNullOrUndefined(item.firm_tel_num2) && isStrEmpty(item.firm_tel_num2) ? <span>/</span> : ""}
                                                </div>
                                                
                                                <div className={styles.sec_phone}>{item.firm_tel_num2}</div>
                                            </div>
                                        </div>
                                        <div className={styles.bottom}>
                                            <div title={item.firm_address} >{item.firm_address.length > 32 ? item.firm_address.slice(0, 32) + '.'.repeat(3) : item.firm_address}</div>
                                            <div className={cx({
                                                fontWeightify: true, 
