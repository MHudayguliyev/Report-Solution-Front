...
/// loading icon
import Loading from '@app/compLibrary/SkeletonCard/MainLoader'
...
import { CheckObjOrArrForNull, isStrEmpty, checkDisconnectedClient, leastFirmConnected, getUserDevice, checkIfArray, arraysChecker } from "@utils/helpers";
...
import { DisconnectedType, ReconnectedType, UsualType } from "@redux/types/TopnavbarTypes";
...
import { DataKindType, LoadingType } from "@app/redux/types/DashboardTypes";
...
type RefetchFNType = {
  ...
  date:{startDate: any, endDate: any} | any
  field?:{value: string, label: Localization}
  /** @defaultValue false */
  renewDashboard?: boolean
  /** @defaultValue false */
  renewDataToCompare?: boolean
  /** @defaultValue false */
  renewReport?:boolean
  /** @defaultValue false */
...
type MessageType = {
  roomName:string
  date: RefetchFNType['date']
  field?: RefetchFNType['field']| string
  list?: {
    endPoint: RefetchFNType['endPoint']
    type_id: number[] | null
  }[]
  endPoint: RefetchFNType['endPoint']
  dashboardDataType?: RefetchFNType['dashboardDataType']
  renewDashboard?: RefetchFNType['renewDashboard']
  renewDataToCompare?: RefetchFNType['renewDataToCompare']
  renewReport?:RefetchFNType['renewReport']
}
...
const RefetchFN = (props:RefetchFNType) => {
  ...
  const message: MessageType = {
    ...
    renewDashboard, renewDataToCompare, 
  }

  if(renewReport){
    delete message.list 
    delete message.renewDashboard
    delete message.renewDataToCompare
    delete message.dashboardDataType
  }
  if(renewDashboard)
    delete message.field

  // console.log("room in message", message)
  socket.emit('request_initial_data', message)
  if(renewDashboard){
    dispatch(DashboardAction.setDashboardSettings({task: 'load', bool: true, dataType:dashboardDataType}))
  ...
  return (
    <div className={styles.topnav__right_user}>
      <div className={styles.topnav__right_user__image}>
        <img src={isStrEmpty(avatar) ?avatar : user_image} alt="user logo" />
      </div>
...
const TopNavbar = () => {
  ...
  const dashboardDataType = useAppSelector(state => state.dashboardReducer.dataType) // initialData/dataToCompare
  ...
  const date = useAppSelector(state => state.topNavbarReducer.dashboardDate)
  const compareDate = useAppSelector(state => state.topNavbarReducer.dashboardCompareDate)
  const startDate = useAppSelector(state => state.topNavbarReducer.reportStartDate)
  const endDate = useAppSelector(state => state.topNavbarReducer.reportEndDate)
  ...
  const loadingMem = useMemo(() => {
    return {
      ...
      stockCostLoading,saleOrdTotalLoading,
      paymentsReceivedLoading,paymentsMadeLoading,
      creditsLoading,debtsLoading,
      employeesBalanceLoading,expensesLoding,
      cashesLoading, isReportsDataLoading,
    }
  }, [
  ...
  const dashboardDataArrays = useMemo(() => {
    return [
        purchSaleOrders.initialData,
        saleOrdTotalByStatus.initialData,
        purchSalesReturns.initialData,
        stockCostTotal.initialData,
        paymentsReceived.initialData,
        paymentsMade.initialData,
        creditsFromSale.initialData,
        debtsFromPurchace.initialData,
        employeesBalance.initialData,
        expensesAmount.initialData,
        cashesAmount.initialData
    ]
  }, [
  ...
  useEffect(() => {
    dispatch(TopnavbarAction.setDashboardCompareDate(
  ...
  useEffect(() => {
    if(
      compareSwitch && 
      (
        !moment(compareDate).isSame(date) && 
        moment(compareDate).isBefore(date)
      )
    ){
      ...
      RefetchFN({
        socket,dispatch,
        receiver,endPoint,
        date: compareDate,renewDataToCompare: true, 
        dashboardDataType: 'dataToCompare'
      })
    }else 
      /// empty dataToCompare when switch off
      dispatch(DashboardAction.setDashboardSettings({task:'emptify', dataType: 'dataToCompare'}))
  }, [
    compareSwitch, 
    compareDate
  ])
  ...
  useEffect(() => {
    if(isReconnected.state){
      refetchFirmsData()
      dispatch(AdminAction.setRefetchState(true)) // refetch firms in admin side once connected
          ...
          if(firm.value === receiverClone.value && !firm.connected){
            for(let j = 0; j < firmsList.length; j++){
              ...
              if(innerFirm.connected){
                ...
                dispatch(TopnavbarAction.setCompareSwitch(false))
    ...
    if(autoRefreshActivated && receiver.connected){
      ...
      if(path==='/dashboard'){
        ...
        const interval = setInterval(() => {
          ...
          if(
              (
                !purchSaleOrdLoading.initialData&&!purchSaleRetLoading.initialData&&
                !stockCostLoading.initialData&&!saleOrdTotalLoading.initialData&&
                !paymentsReceivedLoading.initialData&&!paymentsMadeLoading.initialData&&
                !creditsLoading.initialData&&!debtsLoading.initialData&&!employeesBalanceLoading.initialData&&
                !expensesLoding.initialData&&!cashesLoading.initialData
              ) && 
              (
                !purchSaleOrdLoading.dataToCompare&&!purchSaleRetLoading.dataToCompare&&
                !stockCostLoading.dataToCompare&&!saleOrdTotalLoading.dataToCompare&&
                !paymentsReceivedLoading.dataToCompare&&!paymentsMadeLoading.dataToCompare&&
                !creditsLoading.dataToCompare&&!debtsLoading.dataToCompare&&!employeesBalanceLoading.dataToCompare&&
                !expensesLoding.dataToCompare&&!cashesLoading.dataToCompare
              ) 
          )
            RefetchFN({
              socket,dispatch,
              receiver,endPoint,
              date,renewDashboard: true, 
              dashboardDataType: 'initialData'
            }) 
        }, timer)
  ...
  useEffect(() => {
    if(checkDisconnectedClient(disClient) && CheckObjOrArrForNull(firmsList)){
      ...
      if(compareSwitch)
        dispatch(TopnavbarAction.setCompareSwitch(false))
      dispatch(AdminAction.setRefetchState(true)) // refetch firms in admin side once disconnected
      ...
      if(receiver.connected){
        ...
        if(match.pathname === '/dashboard'){
            if(isDtlTblOpen && !isDetsLoading)
              ...
              dispatch(TopnavbarAction.setRenewDashboard(true))
              dispatch(DashboardAction.setDashboardDataType('initialData')) // note: SET IT TO initialData when click REFRESH
            }else toast.error(t('dataLoading'))
    ...
    return (
      <UserProfile 
        ...
        onDelete={async (userGuid) => {
          try {
              const response:any = await DeleteAvatar(userGuid)
  ...
  const siderbarFilterMenu = (
    <div className={styles.sideBar}>
      ...
      <Row rowGutter={7} colGutter={10}>
        ...
        <Col>
            <Button 
              onClick={() => setShowFirms(true)} 
              style={{padding: '13px',display:'flex',justifyContent:'center',alignContent:'center'}}  
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (selectedDate < min || selectedDate > max) {
                      input.value = "";
                    }else dispatch(TopnavbarAction.setDashboardDate(selectedDate)) 
                  }}
              />
              </div> : 
                  ...
                  if(userSubscData?.subsc_name === "Null")
                    ...
                    if(!receiver.connected)
                      ...
                      if(
                        ...
                        dispatch(TopnavbarAction.setCompareSwitch(!compareSwitch))
                      else if(!arraysChecker(dashboardDataArrays)) toast.error(t('reloadInitialData'))
                      else toast.error(t('dataLoading'))
                    } 
                  }
                }}
              />
              <div className={styles.switchCompareTxt}>
                ...
                <div className={cx({
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if(
                        !moment(selectedDate).isSame(date) && 
                        ...
                        dispatch(TopnavbarAction.setDashboardCompareDate(moment(e.target.value).format("YYYY-MM-DD")))
                    }}
                />
                </div> 
